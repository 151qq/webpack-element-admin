<template>
<div>
  <nav-bar title="干货管理" :crumbs="navBarData"></nav-bar>
  <div class="member-contont">
    <el-form label-width="75px">
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="干货标题:" label-width="75px">
              <el-input placeholder="干货标题" v-model="query.title"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="4">
          <el-form-item label="作者:" label-width="75px">
              <el-input placeholder="作者" v-model="query.author"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
    ...
    <el-table :data="tableData" border stripe width="100%" :height="winHeight">
      ...
      <el-table-column property="amount" label="打赏数"></el-table-column>
      <el-table-column property="views_count" label="阅读量"></el-table-column>
      <el-table-column property="collects_count" label="收藏数"></el-table-column>
      <el-table-column inline-template label="操作" width="80px">
           <el-button-group>
              <i class="el-icon-close handleFour" title="删除干货" @click="closeFn(row.id)"></i>
  ...
  <script>
    ...
    export default {
      ...
      methods: {
        ...
        getData () {
          ywk.getJson('/api/content/article/search', this.query, (res) => {
            if (res.error_code === 0) {
        ...
        // 删除案例
        closeFn (id) {
          this.$confirm('确定要删除该干货吗?', '提示', {
            type: 'warning'
          }).then(() => {
            ywk.postJson(`/api/content/article/remove/${id}`, {}, (res) => {
              if (res.error_code === 0) {
                // 移除该行数据
                let idx = _.findIndex(this.tableData, {id: id})
